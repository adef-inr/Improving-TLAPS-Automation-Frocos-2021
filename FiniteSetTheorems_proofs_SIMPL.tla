---------------------- MODULE FiniteSetTheorems_proofs_SIMPL ----------------------
(***************************************************************************)
  ...
  <1>1. IsCS(SZ)
    ...
    <2>1. \A i \in Nat : Prop(i)
      ...
      <3>2. ASSUME NEW i \in Nat, Prop(i)  PROVE Prop(i+1)
        ...
        <4>. QED  (*BY <4>9, <4>10*)
      (*<3> HIDE DEF Prop*)
      <3> QED BY <3>1, <3>2, NatInduction (* compressed: 1 definition *)
    ...
    <2>1. \A i \in Nat : Prop(i)
      ...
      <3>2. ASSUME NEW i \in Nat, Prop(i)  PROVE Prop(i+1)
        ...
        <4>. SUFFICES ASSUME NEW T \in SUBSET S, ExistsBijection(1 .. i+1,T)
             PROVE CS1[T] = CS2[T]
              (*OBVIOUS*)
        <4>5. ~ExistsBijection(1..0,T)  (*BY Fun_NatBijSame*)
        <4>6. T # {}  (*BY <4>5, Fun_NatBijEmpty*)
        <4>7. PICK t \in T : t = CHOOSE x : x \in T  (*BY <4>6*)
        <4>. DEFINE U == T \ {t}
        <4>9. ExistsBijection(1..i,U)  (*BY Fun_NatBijSubElem*)
...
<1>1. CASE S = {}
  ...
  <2>2. Cardinality(S) = Cardinality(T) <=> f \in Surjection(S,T)
    (*BY <1>1, FS_CardinalityType, FS_EmptySet, Fun_IsSurj, Fun_SurjectionProperties
       DEF Injection*)
  <2>. QED  (*BY <2>1, <2>2*)
<1>2. CASE S # {}
...
<1>3. ASSUME NEW n \in Nat, Q(n),
  ...
  <2>2. /\ IsFiniteSet(T \ {x})
        /\ Cardinality(T \ {x}) = n
    (*BY <1>3, FS_RemoveElement*)
  <2>3. P(T \ {x})  (*BY <2>2, Q(n)*)
  <2>4. P((T \ {x}) \cup {x})  (*BY <2>2, <2>3*)
  <2>. QED  (*BY <2>4*)
<1>4. QED  (*BY <1>2, <1>3, NatInduction*)
...
(*<1>2. IsWellFoundedOn(PreImage(Cardinality, FiniteSubsetsOf(S), OpToRel(<,Nat)),
                       FiniteSubsetsOf(S))
  BY <1>1, PreImageWellFounded, NatLessThanWellFounded*)
...
<1>. QED  (*BY <1>2, <1>3, IsWellFoundedOnSubrelation*)
  BY <1>1, <1>3, PreImageWellFounded, NatLessThanWellFounded, IsWellFoundedOnSubrelation (* compressed: 1 step *)
...
(*<1>. HIDE DEF SubS*)
(*<1>3. \A T \in SubS : P(T)  BY ONLY <1>1, <1>2, WFInduction*)
<1>. QED  (*BY <1>3 DEF SubS*)
  BY FS_FiniteSubsetsOfFinite, FS_StrictSubsetOrderingWellFounded, <1>2, WFInduction (* compressed: 2 steps *)
...
<1>2. ASSUME NEW A, NEW x, IsFiniteSet(A), P(A), x \notin A
      PROVE  P(A \cup {x})
  <2>1. IsFiniteSet(S \cup (A \cup {x}))  (*BY P(A), FS_AddElement*)
  ...
  <2>. QED  (*BY <2>3, <2>4*)
(*<1>. HIDE DEF P*)
(*<1>. P(T)  BY <1>1, <1>2, FS_Induction*)
<1>. QED  (*BY DEF P*)
  BY <1>1, <1>2, FS_Induction (* compressed: 2 steps *)
...
<1>2. ASSUME NEW A, NEW x, IsFiniteSet(A), x \notin A, P(A)
  ...
  <2>3. SUBSET (A \cup {x}) = (SUBSET A) \cup Ax  (*BY <1>2*)
...
\* Last modified Tue Apr 27 15:43:31 CEST 2021 by adefourn
