...
theory tlapm_681489 imports Constant Zenon begin
consts
  "isReal" :: c
  "isa_slas_a" :: "[c,c] => c"
  "isa_bksl_diva" :: "[c,c] => c"
...
(* Generated from file "./Modules/SequenceOpTheorems_proofs_vampire.tla", line 688, characters 3-4 *)
lemma ob'4: (* dad6d2ad7cc34cd21c5d29f9a51b49b0 *)
...
fixes P
fixes S
assumes v'168: "(\<forall> t \<in> ((Seq ((S)))) : (((\<forall> s \<in> ((Seq ((S)))) : ((((IsStrictSuffix ((s), (t)))) \<Rightarrow> ((P ((s))))))) \<Rightarrow> ((P ((t)))))))"
assumes v'169: "((\<And> P_1 :: c => c. (\<And> R :: c. (\<And> S_1 :: c. (((IsWellFoundedOn ((R), (S_1)))) \<Longrightarrow> ((\<forall> x \<in> (S_1) : (((\<forall> y \<in> (subsetOf((S_1), %y. (((<<(y), (x)>>) \<in> (R))))) : ((P_1 ((y))))) \<Rightarrow> ((P_1 ((x))))))) \<Longrightarrow> (\<forall> x \<in> (S_1) : ((P_1 ((x)))))))))))"
assumes v'170: "((\<And> S_1 :: c. ((IsWellFoundedOn ((subsetOf(((((Seq ((S_1)))) \<times> ((Seq ((S_1)))))), %ss. ((IsStrictSuffix ((fapply ((ss), ((Succ[0])))), (fapply ((ss), ((Succ[Succ[0]]))))))))), ((Seq ((S_1)))))))))"
shows "(\<forall> s \<in> ((Seq ((S)))) : ((P ((s)))))"(is "PROP ?ob'4")
proof -
show "PROP ?ob'4"
using assms by auto
qed
