...
theory tlapm_804019 imports Constant Zenon begin
consts
  "isReal" :: c
  "isa_slas_a" :: "[c,c] => c"
  "isa_bksl_diva" :: "[c,c] => c"
...
(* Generated from file "./Modules/SequenceOpTheorems_proofs.tla", line 470, characters 11-12 *)
lemma ob'12: (* 5d5c2a6be75bc559241cd33d1cb0b866 *)
...
fixes P
fixes S
assumes v'155: "(\<forall> t \<in> ((Seq ((S)))) : (((\<forall> s \<in> ((Seq ((S)))) : ((((IsStrictPrefix ((s), (t)))) \<Rightarrow> ((P ((s))))))) \<Rightarrow> ((P ((t)))))))"
assumes v'157: "((\<And> P_1 :: c => c. (\<And> R :: c. (\<And> S_1 :: c. (((IsWellFoundedOn ((R), (S_1)))) \<Longrightarrow> ((\<forall> x \<in> (S_1) : (((\<forall> y \<in> ((SetLessThan ((x), (R), (S_1)))) : ((P_1 ((y))))) \<Rightarrow> ((P_1 ((x))))))) \<Longrightarrow> (\<forall> x \<in> (S_1) : ((P_1 ((x)))))))))))"
assumes v'158: "((\<And> S_1 :: c. ((IsWellFoundedOn (((OpToRel ((IsStrictPrefix), ((Seq ((S_1))))))), ((Seq ((S_1)))))))))"
assumes v'159: "(\<forall> t \<in> ((Seq ((S)))) : (((\<forall> s \<in> ((SetLessThan ((t), ((OpToRel ((IsStrictPrefix), ((Seq ((S))))))), ((Seq ((S))))))) : ((P ((s))))) \<Rightarrow> ((P ((t)))))))"
shows "(\<forall> s \<in> ((Seq ((S)))) : ((P ((s)))))"(is "PROP ?ob'12")
proof -
show "PROP ?ob'12"
using assms by auto
qed
