...
theory tlapm_bf40c5 imports Constant Zenon begin
consts
  "isReal" :: c
  "isa_slas_a" :: "[c,c] => c"
  "isa_bksl_diva" :: "[c,c] => c"
...
(* Generated from file "./Modules/SequenceOpTheorems_proofs.tla", line 765, characters 3-4 *)
lemma ob'45: (* 2e12b73f4e2a69ef60ddcb367b83570a *)
...
assumes v'188: "((IsWellFoundedOn (((OpToRel ((IsStrictSuffix), ((Seq ((S))))))), ((Seq ((S)))))))"
assumes v'189: "((WFDefOn (((OpToRel ((IsStrictSuffix), ((Seq ((S))))))), ((Seq ((S)))), (Def))))"
assumes v'190: "((\<And> Def_1 :: c => c => c. (\<And> f_1 :: c. (\<And> R :: c. (\<And> S_1 :: c. (\<And> T_1 :: c. ((((T_1) \<noteq> ({}))) \<Longrightarrow> (((IsWellFoundedOn ((R), (S_1)))) \<Longrightarrow> (((WFDefOn ((R), (S_1), (Def_1)))) \<Longrightarrow> (((WFInductiveDefines ((f_1), (S_1), (Def_1)))) \<Longrightarrow> ((\<forall> g \<in> ([(S_1) \<rightarrow> (T_1)]) : (\<forall> s \<in> (S_1) : ((((Def_1 ((g), (s)))) \<in> (T_1))))) \<Longrightarrow> (((f_1) \<in> ([(S_1) \<rightarrow> (T_1)]))))))))))))))"
shows "(((f) \<in> ([((Seq ((S)))) \<rightarrow> (T)])))"(is "PROP ?ob'45")
proof -
show "PROP ?ob'45"
using assms by auto
qed
