--------------------- MODULE SequenceOpTheorems_proofs_zipper ---------------------
(**************************************************************************)
...
<1>3. ASSUME NEW j \in 1 .. Len(seq)+1
      PROVE  InsertAt(seq,i,elt)[j] = IF j<i THEN seq[j]
                                      ELSE IF j=i THEN elt
                                      ELSE seq[j-1]
  <2>. j \in 1 .. Len(left)+1+Len(right)  (*BY <1>1, <1>2*)
...
<1>3. ASSUME NEW j \in 1 .. Len(seq)-1
  ...
  <2>. j \in 1 .. Len(left) + Len(right)  (*BY <1>1, <1>2*)
...
<1>2. ASSUME NEW n \in Nat,  Q(n) 
  ...
  <2>1. /\ Front(s) \in Seq(S)
    ...
    (*BY FrontProperties*)
  <2>. QED  (*BY <2>1, <1>2*)
<1>3. QED  (*BY <1>1, <1>2, NatInduction*)
...
<1>1. /\ s \o t \in Seq(S)
      /\ IsPrefix(s, s \o t)
  (*BY IsPrefixConcat*)
<1>. QED  (*BY <1>1, IsPrefixPartialOrder*)
...
(*BY <<e>> \in Seq(S), ConcatIsPrefixCancel DEF Cons*)
...
<1>. QED  BY WFInduction, IsStrictPrefixWellFounded, <1>1*)
  BY WFInduction, IsStrictPrefixWellFounded DEF SetLessThan, OpToRel (* compressed: 1 step *)
...
<1>3. IsSuffix(s,t) <=> IsPrefix(Reverse(s), Reverse(t))
  ...
  <2>1. ASSUME IsSuffix(s,t)
    ...
    <3>2. /\ Reverse(u) \in Seq(S)
      ...
      (*BY <3>1, ReverseProperties, ReverseConcat*)
    <3>. QED  (*BY <3>2, IsPrefixProperties*)
  <2>2. ASSUME IsPrefix(Reverse(s), Reverse(t))
    ...
    <3>2. /\ Reverse(u) \in Seq(S)
      ...
      (*BY <3>1, ReverseProperties, ReverseConcat*)
    <3>. QED  (*BY <3>2, <1>1, ReverseProperties*)
  <2>. QED  (*BY <2>1, <2>2*)
<1>. QED  (*BY <1>1, <1>2, <1>3*)
...
(*BY IsSuffixProperties, ConcatProperties*)
...
<1>. /\ <<e>> \in Seq(S)
     /\ <<e>> \o s \in Seq(S)
  (*OBVIOUS*)
<1>. QED  (*BY IsStrictSuffixProperties DEF Cons*)

THEOREM TailIsSuffix ==
  ASSUME NEW S, NEW s \in Seq(S), s # << >>
  PROVE  IsStrictSuffix(Tail(s), s)
(*BY ConsHeadTail, IsStrictSuffixCons, Head(s) \in S, Tail(s) \in Seq(S)*)
...
(*BY <<e>> \in Seq(S), ConcatIsSuffixCancel, AppendIsConcat*)
...
<1>. <<e>> \in Seq(S)
  (*OBVIOUS*)
<1>1. IsSuffix(<<e>>, u)
  (*BY ConcatIsSuffix, AppendIsConcat*)
...
(*<1>1. \A t \in Seq(S) : 
         ...
         => P(t)
  BY DEF SetLessThan, OpToRel
<1>. QED  BY WFInduction, IsStrictSuffixWellFounded, <1>1*)
  BY WFInduction, IsStrictSuffixWellFounded DEF SetLessThan, OpToRel (* compressed: 1 step *)
...
(*BY DEF StrictPrefixesDetermineDef, WFDefOn, OpToRel, SetLessThan*)
...
<1>. QED
  ...
  BY IsStrictPrefixWellFounded, StrictPrefixesDetermineDef_WFDefOn, WFInductiveDefType (* compressed: 2 steps *)
...
(*BY DEF StrictSuffixesDetermineDef, WFDefOn, OpToRel, SetLessThan*)
...
<1>. QED
  BY <1>1, <1>2, WFInductiveDefType*)
  BY IsStrictSuffixWellFounded, StrictSuffixesDetermineDef_WFDefOn, WFInductiveDefType (* compressed: 2 steps *)
...
<1>1. StrictSuffixesDetermineDef(S, Op)
  <2>. SUFFICES ASSUME NEW g, NEW h, NEW seq \in Seq(S),
    ...
    BY DEF StrictSuffixesDetermineDef
  <2>1. CASE seq = <<>>
    BY <2>1
  <2>2. CASE seq # <<>>
    BY <2>2, TailIsSuffix, Tail(seq) \in Seq(S)
  <2>. QED  BY <2>1, <2>2
...
<1>. QED  BY <1>3 DEF WFInductiveDefines, TailInductiveDefConclusion*)
  BY TailIsSuffix, SuffixRecursiveSequenceFunctionDef DEF StrictSuffixesDetermineDef, OpDefinesFcn, TailInductiveDefHypothesis, WFInductiveDefines, TailInductiveDefConclusion (* compressed: 7 steps *)
...
<1>. QED  (*BY <1>1, <1>2, SequencesInductionCons*)
...
<1>1. StrictPrefixesDetermineDef(S, Op)
  <2>. SUFFICES ASSUME NEW g, NEW h, NEW seq \in Seq(S),
                       \A pre \in Seq(S) : IsStrictPrefix(pre, seq) => g[pre] = h[pre]
                PROVE  Op(g, seq) = Op(h, seq)
    BY DEF StrictPrefixesDetermineDef
  <2>1. CASE seq = <<>>
    BY <2>1
  <2>2. CASE seq # <<>>
...
<1>. QED  BY <1>3 DEF WFInductiveDefines, FrontInductiveDefConclusion*)
  BY FrontIsPrefix, FrontProperties, PrefixRecursiveSequenceFunctionDef DEF StrictPrefixesDetermineDef, OpDefinesFcn, FrontInductiveDefHypothesis, WFInductiveDefines, FrontInductiveDefConclusion (* compressed: 6 steps *)
...
<1>2. ASSUME NEW seq \in Seq(S), NEW e \in S, f[seq] \in T
  ...
  <2>. QED  (*BY <1>2, <2>1, <2>2*)
<1>. QED  (*BY <1>1, <1>2, SequencesInductionAppend*)
