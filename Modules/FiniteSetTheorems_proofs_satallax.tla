---------------------- MODULE FiniteSetTheorems_proofs_satallax ----------------------
(***************************************************************************)
  ...
  <1>1. IsCS(SZ)
    ...
    <2>1. \A i \in Nat : Prop(i)
      ...
      <3>2. ASSUME NEW i \in Nat, Prop(i)  PROVE Prop(i+1)
        ...
        <4>. QED  (*BY <4>9, <4>10*)
      (*<3> HIDE DEF Prop*)
      <3> QED BY <3>1, <3>2, NatInduction (* compressed: 1 step *)
...
<1>. QED  (*BY <1>2, <1>3, IsWellFoundedOnSubrelation*)
  BY <1>1, PreImageWellFounded, NatLessThanWellFounded, <1>3, IsWellFoundedOnSubrelation (* compressed: 1 step *)
...
<1>. QED  (*BY <1>3 DEF SubS*)
  BY FS_FiniteSubsetsOfFinite, FS_StrictSubsetOrderingWellFounded, <1>2, WFInduction (* compressed: 3 steps *)
...
<1>2. ASSUME NEW A, NEW x, IsFiniteSet(A), P(A), x \notin A
  ...
  <2>. QED  (*BY <2>3, <2>4*)
(*<1>. HIDE DEF P*)
<1>. P(T)  BY <1>1, <1>2, FS_Induction (* compressed: 1 step *)
<1>. QED  (*BY DEF P*)
...
<1>2. ASSUME NEW U, NEW x, P(U), x \notin U
  ...
  (*BY <1>2, FS_Union*)
(*<1>. HIDE DEF P*)
<1>. P(S)  BY <1>1, <1>2, FS_Induction (* compressed: 1 step *)
<1>. QED  (*BY DEF P*)
...
\* Last modified Fri Jul 09 00:10:57 CEST 2021 by adefourn
