--------------------- MODULE WellFoundedInduction_proofs_vampire --------------------
(***************************************************************************)
...
<1>3. \A x \in S : SetLessThan(x, R, S) \subseteq SetLessThan(x, TC, S)
...
<1>4. QED 
 (*BY <1>1, <1>2, <1>3, WFInductiveDefLemma*)
  BY TransitiveClosureThm, TransitiveClosureWF, <1>3, WFInductiveDefLemma DEF WFDefOn (* compressed: 3 steps *)
...
<1>1. \A s \in S : f[s] \in T
  <2>1. SUFFICES ASSUME NEW s \in S,
               ...
               PROVE  f[s] \in T
    (*BY ONLY <2>1, IsWellFoundedOn(R, S), WFInduction*)
  <2>2. PICK t0 : t0 \in T
    (*OBVIOUS*)
...
<1>3. ASSUME NEW n \in Nat, 
  ...
  <2>1. g \in [Nat -> Nat]
    (*BY ONLY f \in [Nat -> Nat]*)
  <2>2. \A i \in Nat : <<g[i+1], g[i]>> \in R
    (*OBVIOUS*)
  <2>3. g[0] \in 0..(n-1)
    <3>1. f[1] < f[0]  (*BY DEF OpToRel*)
    <3>2. /\ f[1] \in Nat
          /\ f[1] < n
      (*BY <3>1*)
    <3>3. f[1] \in 0 .. (n-1)
...
<1>4. ~ P(ff[0])
  <2> HIDE DEF P
  <2> \A n \in Nat : ~ P(n)
    (*BY ONLY <1>3, GeneralNatInduction*)
  <2> QED
    (*BY DEF P*)
<1>5. QED
    (*BY <1>2, <1>4*)
...
<1>1. P(0)
  (*BY EmptyIsWellFounded, 1..0 = {} DEF LexProductOrdering*)
<1>2. ASSUME NEW m \in Nat, P(m)
  ...
  <2>2. IsWellFoundedOn(PreImage(g, [1..m+1 -> S], LexPairOrdering(LPO(m), R, [1..m -> S], S)),
    ...
    (*<3> HIDE DEF g*)
    <3>2. QED
      BY <2>1, <3>1, PreImageWellFounded (* compressed: 1 step *)
    ...
    <3>2. PreImage(g, [1..m+1 -> S], LexPairOrdering(LPO(m), R, [1..m -> S], S)) \subseteq LPO(m+1)
      ...
      <4>1. CASE << g(x)[1], g(y)[1] >> \in LPO(m)
        ...
        <5>3. /\ g(x)[1][j] = x[j]
          ...
          (*BY DEF g*)
        <5> QED
          (*BY <5>1, <5>3, <3>0 DEF LexProductOrdering*)
...
\* Last modified Fri Jul 09 15:54:36 CEST 2021 by adefourn
