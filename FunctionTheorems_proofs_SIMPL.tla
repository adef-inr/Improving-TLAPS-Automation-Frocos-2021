--------------------- MODULE FunctionTheorems_proofs_SIMPL ------------------------
(***************************************************************************)
(* `^{\large \vspace{12pt}                                                 *)
...
<1>2. g \in [T -> S]  (*BY <1>1, Fun_InverseProperties*)
...
<1>3. \A i \in Nat : Ci[i] = IF i = 0 THEN Y ELSE {F[s] : s \in Ci[i-1]}
  ...
  <2> QED (*BY DEF NatInductiveDefHypothesis, f*)
    BY NatInductiveDef DEF NatInductiveDefConclusion, NatInductiveDefHypothesis, f (* compressed: 2 steps *)
...
<1>6. \A i \in Nat : Ci[i] \subseteq S
  ...
  (*<2> HIDE DEF Prop*)
  <2> QED BY <2>1, <2>2, NatInduction (* compressed: 1 definition *)
...
<1>11. C = Y \cup FC
  <2>1. ASSUME NEW c \in C  PROVE c \in Y \cup FC
    ...
    <3>3. CASE i # 0
      (* old proof fails:
      <4>1. PICK s \in Ci[i-1] : F[s] = c  (*BY <3>1, <3>3, <1>5*)
      *)
...
<1>1. PICK m \in Nat :
  ...
  BY WFMin, NatLessThanWellFounded DEF OpToRel (* compressed: 1 step, 1 definition *)
...
<1>4. ASSUME f \notin Injection(1..m, S)  PROVE FALSE
  ...
  <2>. QED  (*BY <2>3, <2>4, <1>1 DEF ExistsSurjection*)
...
\* Last modified Tue Apr 27 17:56:14 CEST 2021 by adefourn
